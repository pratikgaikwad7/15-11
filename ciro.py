...
@ciro_bp.app_template_filter('format_date')
...
def dashboard():
    try:
        ...
        SELECT 
            ...
            COALESCE(AVG((sec1_q1 + sec1_q2 + sec2_q1 + sec2_q2 + sec2_q3 + sec3_q1 + 
                sec5_q1 + sec5_q2 + sec6_q1 + sec6_q2 + sec7_q1 + sec7_q2)/12.0), 0) as csi,
            COALESCE((
            ...
            COALESCE((COALESCE(AVG((sec1_q1 + sec1_q2 + sec2_q1 + sec2_q2 + sec2_q3 + sec3_q1 + 
                sec5_q1 + sec5_q2 + sec6_q1 + sec6_q2 + sec7_q1 + sec7_q2)/12.0), 0) + 
                COALESCE((
                    SELECT AVG(avg_score)
                    FROM (
                        SELECT (AVG(trainer1_q1) + AVG(trainer1_q2) + AVG(trainer1_q3) + AVG(trainer1_q4))/4 as avg_score
        ...
        for session in sessions:
            # Handle program_date formatting
            try:
                if session['program_date']:
                    session['formatted_program_date'] = session['program_date'].strftime('%d-%m-%Y')
                else:
                    session['formatted_program_date'] = "N/A"
            except (AttributeError, TypeError):
                session['formatted_program_date'] = "N/A"
            
            # Handle category formatting
            session['pmo_training_category'] = session['pmo_training_category'] or "N/A"
            session['pl_category'] = session['pl_category'] or "N/A"
            session['brsr_sq_123_category'] = session['brsr_sq_123_category'] or "N/A"
        ...
        # Get unique years for filter dropdown - handle NULL dates
        cursor.execute("""
            SELECT DISTINCT YEAR(program_date) as year 
            FROM feedback_responses 
            WHERE program_date IS NOT NULL 
            ORDER BY year DESC
        """)
...
def training_detail(program_title, program_date):
    ...
    # Format dates for all programs
    for program in all_programs:
        try:
            if program['program_date']:
                program['formatted_program_date'] = program['program_date'].strftime('%d-%m-%Y')
            else:
                program['formatted_program_date'] = "N/A"
        except (AttributeError, TypeError):
            program['formatted_program_date'] = "N/A"
    ...
    SELECT 
        ...
        COALESCE(AVG((sec1_q1 + sec1_q2 + sec2_q1 + sec2_q2 + sec2_q3 + sec3_q1 + sec5_q1 + sec5_q2 + sec6_q1 + sec6_q2 + sec7_q1 + sec7_q2)/12.0), 0) as csi,
        COALESCE((
            SELECT AVG(avg_score)
            FROM (
                SELECT (AVG(trainer1_q1) + AVG(trainer1_q2) + AVG(trainer1_q3) + AVG(trainer1_q4))/4 as avg_score
        ...
        COALESCE((COALESCE(AVG((sec1_q1 + sec1_q2 + sec2_q1 + sec2_q2 + sec2_q3 + sec3_q1 + sec5_q1 + sec5_q2 + sec6_q1 + sec6_q2 + sec7_q1 + sec7_q2)/12.0), 0) + 
        COALESCE((
            SELECT AVG(avg_score)
            FROM (
                SELECT (AVG(trainer1_q1) + AVG(trainer1_q2) + AVG(trainer1_q3) + AVG(trainer1_q4))/4 as avg_score
    ...
    # Prepare arguments - we need 18 arguments (9 pairs of program_title, program_date)
    args = (program_title, program_date, program_title, program_date, program_title, program_date, 
            program_title, program_date, program_title, program_date, program_title, program_date,
            program_title, program_date, program_title, program_date, program_title, program_date)
    ...
    trainer_args = (program_title, program_date, program_title, program_date, 
                   program_title, program_date, program_title, program_date)
    cursor.execute(trainer_query, trainer_args)
    trainers = cursor.fetchall()
    
    # Get all individual responses with all questions
    individual_query = """
        SELECT *,
            (COALESCE(sec1_q1,0) + COALESCE(sec1_q2,0) + COALESCE(sec2_q1,0) + COALESCE(sec2_q2,0) + COALESCE(sec2_q3,0) + COALESCE(sec3_q1,0) + 
             COALESCE(sec5_q1,0) + COALESCE(sec5_q2,0) + COALESCE(sec6_q1,0) + COALESCE(sec6_q2,0) + COALESCE(sec7_q1,0) + COALESCE(sec7_q2,0))/12.0 as csi_score,
            (
    ...
    cursor.execute(individual_query, (program_title, program_date))
    individual_responses = cursor.fetchall()
    
    # Format individual responses
    for response in individual_responses:
        try:
            if response['program_date']:
                response['formatted_program_date'] = response['program_date'].strftime('%d-%m-%Y')
            else:
                response['formatted_program_date'] = "N/A"
        except (AttributeError, TypeError):
            response['formatted_program_date'] = "N/A"
        
        try:
            if response['created_at']:
                response['formatted_created_at'] = response['created_at'].strftime('%d-%m-%Y %H:%M:%S')
            else:
                response['formatted_created_at'] = "N/A"
        except (AttributeError, TypeError):
            response['formatted_created_at'] = "N/A"
    ...
    if clubbed_session_ids:
        ...
        for session_id in clubbed_session_ids:
            ...
            cursor.execute(clubbed_detail_query, (session_id['clubbed_session_id'],))
            clubbed_info.extend(cursor.fetchall())
        
        # Format clubbed session dates
        for info in clubbed_info:
            try:
                if info['program_date']:
                    info['formatted_program_date'] = info['program_date'].strftime('%d-%m-%Y')
                else:
                    info['formatted_program_date'] = "N/A"
            except (AttributeError, TypeError):
                info['formatted_program_date'] = "N/A"
...
def export_summary():
    ...
    try:
        ...
        # Format dates in DataFrame - handle None values
        df['program_date'] = df['program_date'].apply(lambda x: x.strftime('%d-%m-%Y') if pd.notna(x) else "N/A")
...
def export_summary_report(program_title, program_date):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            cursor.execute(summary_query, args)
            ...
            # Format dates in DataFrame - handle None values
            summary_df['program_date'] = summary_df['program_date'].apply(lambda x: x.strftime('%d-%m-%Y') if pd.notna(x) else "N/A")
